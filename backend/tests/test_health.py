...
@pytest.mark.asyncio
async def test_readiness_reports_heuristic_mode_without_an_api_key(client, monkeypatch):
    """A missing Anthropic key is a supported mode, not a failure.

    Triage falls back to the keyword heuristic, so readiness must stay 200
    and merely say which engine is live — otherwise a free deploy with no
